...
import { getTransfers, updateTransfer, validateTransfer, cancelTransfer } from '../api/transfers';
import { getProducts } from '../api/products';
import { getWarehouses } from '../api/warehouses';
import { useToast } from '../context/ToastContext';
import { ArrowLeft, Save, CheckCircle, XCircle, Loader2, Trash2, Plus, Check, ArrowRight, Info } from 'lucide-react';
...
export default function TransferDetail() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [tRes, pRes, wRes] = await Promise.all([getTransfers({}), getProducts(), getWarehouses()]);
      const data = tRes.data.data.find(t => t._id === id);
      if (!data) { toast.error('Transfer not found'); navigate('/transfers'); return; }
      setTransfer(data); setProductsList(pRes.data.data || []); setWarehousesList(wRes.data.data || []);
    ...
    } catch (err) { toast.error('Failed to load transfer'); } finally { setLoading(false); }
  };
  ...
  const currentStep = isDone ? 1 : 0;

  const handleSave = async () => {
    try {
      setSaving(true);
      if (!fromWarehouse || !toWarehouse) throw new Error('Both warehouses required');
      if (fromWarehouse === toWarehouse) throw new Error('Must be different warehouses');
      await updateTransfer(id, { fromWarehouse, toWarehouse, scheduledDate: scheduledDate ? new Date(scheduledDate).toISOString() : null, products: lines.filter(l => l.productId) });
      toast.success('Transfer saved'); await loadData();
    } catch (err) { toast.error(err.response?.data?.message || err.message || 'Failed to save'); } finally { setSaving(false); }
  };

  const handleAction = async (action) => {
    try {
      setLoading(true);
      if (canEdit && action === 'validate') {
        if (!fromWarehouse || !toWarehouse) throw new Error('Both warehouses required');
        if (fromWarehouse === toWarehouse) throw new Error('Must be different warehouses');
        await updateTransfer(id, { fromWarehouse, toWarehouse, scheduledDate: scheduledDate ? new Date(scheduledDate).toISOString() : null, products: lines.filter(l => l.productId) });
      }
      if (action === 'validate') await validateTransfer(id);
      else if (action === 'cancel') await cancelTransfer(id);
      toast.success(`Transfer ${action}d`); await loadData();
    } catch (err) { toast.error(err.response?.data?.message || err.message || `Failed to ${action}`); } finally { setLoading(false); }
  };
  ...
  if (loading && !transfer) return <div className="flex items-center justify-center min-h-[400px]"><Loader2 className="w-8 h-8 text-indigo-600 animate-spin" /></div>;

  return (
    <div className="max-w-[1000px] mx-auto pb-12">
      <div className="flex items-center gap-4 mb-6">
        <button onClick={() => navigate('/transfers')} className="p-2 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 text-slate-600 transition-colors"><ArrowLeft size={20} /></button>
        <div>
          <p className="text-slate-500 text-sm">Transfers / {transfer?.referenceNo}</p>
          <h1 className="text-xl font-bold text-slate-900">{transfer?.referenceNo || 'Draft Transfer'}</h1>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 mb-6">
        {isDraft && (<>
          ...
          <button onClick={() => handleAction('validate')} className="flex items-center gap-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium rounded-lg transition-all"><CheckCircle size={16} /> Validate</button>
        </>)}
        ...
        {isDraft && <button onClick={() => handleAction('cancel')} className="flex items-center gap-2 px-4 py-2 border border-slate-300 text-slate-700 hover:bg-red-50 hover:text-red-700 hover:border-red-200 text-sm font-medium rounded-lg transition-all"><XCircle size={16} /> Cancel</button>}
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm p-6 mb-6">
        <div className="grid grid-cols-1 md:grid-cols-[1fr,auto,1fr] gap-4 items-end mb-6">
          <div>
            <label className="block text-[11px] font-semibold text-slate-500 uppercase tracking-wider mb-1.5">From Warehouse</label>
            {canEdit ? <select value={fromWarehouse} onChange={e => setFromWarehouse(e.target.value)} className="w-full h-10 px-3 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-indigo-500 focus:ring-[3px] focus:ring-indigo-500/15 appearance-none bg-white">
              <option value="" disabled>Select source...</option>{warehousesList.map(w => <option key={w._id} value={w._id}>{w.name}</option>)}
            </select> : <p className="text-slate-900 font-medium">{warehousesList.find(w => w._id === fromWarehouse)?.name || '-'}</p>}
          </div>
          <div className="flex justify-center py-2"><ArrowRight size={24} className="text-indigo-400" /></div>
          <div>
            <label className="block text-[11px] font-semibold text-slate-500 uppercase tracking-wider mb-1.5">To Warehouse</label>
            {canEdit ? <select value={toWarehouse} onChange={e => setToWarehouse(e.target.value)} className="w-full h-10 px-3 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-indigo-500 focus:ring-[3px] focus:ring-indigo-500/15 appearance-none bg-white">
              <option value="" disabled>Select destination...</option>{warehousesList.map(w => <option key={w._id} value={w._id}>{w.name}</option>)}
            </select> : <p className="text-slate-900 font-medium">{warehousesList.find(w => w._id === toWarehouse)?.name || '-'}</p>}
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead><tr className="bg-slate-50">
              ...
              <th className="text-left px-6 py-3 text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Quantity</th>
      ...
      {!isDone && !isCanceled && lines.some(l => l.productId) && (
        <div className="mt-6 flex items-start gap-3 p-4 bg-purple-50 border-l-4 border-purple-600 rounded-r-xl">
          <Info size={18} className="text-purple-600 flex-shrink-0 mt-0.5" />
          <p className="text-sm text-purple-700">Validating will <strong>move stock</strong> from the source to the destination warehouse.</p>
        </div>
      )}
    </div>
  );
